...
#define cm_5 16738455 //"1"
#define cm_10 16750695 // "2"
#define cm_15 16756815 // "3"
...
// float piderror = 0.0;
// float prevpiderror = 0.0;
// float integral = 0.0;
// float derivative = 0.0;
unsigned long prevYawMillis = millis();
unsigned long prevDistMillis = millis();
float prevYawError = 0.0;
float prevDistError = 0.0;
// float dt = 0.0; //Derivative time
...
void handleRemoteCommand(unsigned long command) {
    ...
    {
      case cm_5:
        Serial.println("Set Point = 5cm");
        setPoint = 5;
        digitalWrite(PIN_Motor_STBY, HIGH);
        break;

      case cm_10:
        Serial.println("Set Point = 10cm");
        setPoint = 10;
        digitalWrite(PIN_Motor_STBY, HIGH);
        break;

      case cm_15:
        Serial.println("Set Point = 15cm");
        setPoint = 15;
        digitalWrite(PIN_Motor_STBY, HIGH);
        break;
...
float yawPID()
{
  ...
  float dt = (currentMillis - prevYawMillis) / 1000.0;
  prevYawMillis = currentMillis;

  float currentYaw = round(get_yaw());  
  float error =   targetYaw - currentYaw;
  float integtalError = integtalError + error * dt;
  float derivativeError = (error - prevYawError) / dt;
  prevYawError = error;
    
  float outPID = kp * error + ki * integtalError + kd * derivativeError;
  return outPID;
}

float distPID() {
  float currentMillis = millis();
  float dt = (currentMillis - prevDistMillis) / 1000.0;
  prevDistMillis = currentMillis;

  float currentDist = get_distance();  
  float error =   setPoint - currentDist;
  float integtalError = integtalError + error * dt;
  float derivativeError = (error - prevDistError) / dt;
  prevDistError = error;
...
void moveSetPoint(int speed) //Open Straight-line Paths 
{
  ...
  while (distPID() != 0) //distance != setPoint
  {
    if(distPID() < 0) //distance > setPoint
    {
      moveForwardGyro(speed);
      distance = get_distance();
    }
    else if (distPID() > 0) //distance < setPoint
    {
      reverseGyro(speed);
      distance = get_distance();
    }
