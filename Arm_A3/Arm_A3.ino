...
void loop() {
  ...
  if (digitalRead(CTRL_RIGHT_SW) == LOW && digitalRead(CTRL_LEFT_SW) == LOW) { // LOW
    ...
    while (digitalRead(CTRL_RIGHT_SW) == LOW && digitalRead(CTRL_LEFT_SW) == LOW) { // LOW
      // Wait for both buttons to be released
    delay(100);
    }
  }
  if (digitalRead(CTRL_LEFT_SW)== LOW && digitalRead(CTRL_RIGHT_SW) == HIGH && stepCount > 0 && recordMode == false) {
    playMode = !playMode;
    if (playMode) {
